...
def dream_generate_with_visualization(
    history, max_new_tokens, steps, temperature, top_p, top_k, delay, alg, alg_temp
):
    print("\n--- Starting dream_generate_with_visualization ---")
    print(
        f"Parameters: max_new_tokens={max_new_tokens}, steps={steps}, temperature={temperature}, top_p={top_p}, top_k={top_k}, delay={delay}, alg={alg}, alg_temp={alg_temp}"
    )

    messages_for_model = format_gradio_history_to_messages(history)
    print("Succesfully turned gradio history into LLM message format.")

    print("Attempt to apply chat template")
    try:
        ...
        yield (
            format_gradio_history_to_messages(current_history),
            error_message,
            current_history,
        )
        return
    print("Succeeded in applying chat template")
    ...
    # 开启新线程进行模型推理
    print("Attempt to generate, starting another thread to launch generation function...")
    ...
    # 确定生成部分的长度
    print("Waiting until something changes")
    ...
    # 主循环：不断检查是否有新的中间状态
    print("Listening to the result of other thread")
    while gen_thread.is_alive() or last_yielded < len(visualization_token_states):
        ...
        while last_yielded < current_length:
            ...
            messages_for_chatbot_update = format_gradio_history_to_messages(
...
def bot_response_generator(
    ...
    if not history or history[-1][1] is not None:
        print(
            "Skipping bot response: No history or last message already has a response."
        )
...
with gr.Blocks(css=css, theme=gr.themes.Soft()) as demo: # type: ignore
    ...
    gr.Markdown(
        "Model link: [Dream-org/Dream-v0-Instruct-7B](https://huggingface.co/Dream-org/Dream-v0-Instruct-7B)"
    )
    ...
    with gr.Row():
        with gr.Column(scale=3):
            chatbot_display = gr.Chatbot(
                label="Chat", bubble_full_width=False, height=600, type="messages"
            )
            with gr.Group():
                with gr.Row():
    ...
    with gr.Accordion("Generation Parameters", open=False):
        ...
        top_p_slider = gr.Slider(
            0.0, 1.0, value=0.95, step=0.05, label="Top-p (0 = disabled)"
        )
        top_k_slider = gr.Slider(0, 100, value=0, step=1, label="Top-k (0 = disabled)")
        delay_slider = gr.Slider(
            0.0, 0.5, value=0.02, step=0.01, label="Visualization Delay (seconds)"
        )
        ...
        alg_temp_slider = gr.Slider(
            0.0, 1.0, value=0.1, step=0.01, label="Algorithm Temperature (alg_temp)"
        )
