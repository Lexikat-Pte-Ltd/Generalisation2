...
def get_storage_info(container) -> Tuple[int, int]:
    ...
    try:
        ...
        if exit_code == 0 and isinstance(output, bytes):
            ...
            raise Exception(
                "Failed to retrieve storage information caused by `df -m /`"
            )
...
def safe_detect_env(client: DockerClient, container_id: str) -> EnvironmentInfo:
    ...
    try:
        ...
        logger.error(
            f"Error detecting environment for container {container_id[:12]}: {e}"
        )
        raise e
...
def write_code_in_con(
    ...
    logger.debug(f"Writing file {temp_file_name} into host machine")
    ...
    logger.debug(f"Writing file {temp_file_name} into container")
...
def write_and_run_code_in_con(
    ...
    try:
        logger.info("Executing the written code with a timeout...")
        ...
        logger.error(f"Run code in container got timeout: \n{e}")
        container.exec_run(cmd="kill -9 $(pidof python)")
        return -1, str(e), ""
    except docker.errors.ContainerError as e:
        logger.error(f"Run code in container failed: \n{e}")
        return -1, str(e), ""
...
def write_and_run_code_in_con_v2(
    ...
    # Create a result queue to communicate between threads
    ...
    def run_in_thread():
        try:
            ...
            python_exit_code, python_output = cast(
                Tuple[int, bytes],
                container.exec_run(
                    cmd=["python", "-u", temp_file_name, "2>&1"],
                    demux=False,  # Combine stdout and stderr
                    stream=False,  # Wait for the command to finish and return all output at once
                ),
            )
            # Put the result in the queue
            result_queue.put(("success", python_exit_code, python_output))
        except docker.errors.ContainerError as e:
            # Put the error in the queue
            result_queue.put(("container_error", -1, str(e).encode()))
        except Exception as e:
            # Put any other error in the queue
            result_queue.put(("error", -1, str(e).encode()))

    # Start the execution in a thread
    execution_thread = threading.Thread(target=run_in_thread)
    execution_thread.daemon = True
    execution_thread.start()

    # Wait for result with timeout
    execution_thread.join(150)  # Wait up to 150 seconds

    # Check if we have a result
    if not result_queue.empty():
        # Get the result from the queue
        status, exit_code, output = result_queue.get()
        ...
        elif status == "container_error":
            logger.error(
                f"Run code in container failed: \n{output.decode('utf-8', errors='replace')}"
            )
        ...
        logger.error(
            "Run code in container got timeout: Execution timed out after 150 seconds"
        )

        # Try to kill the Python process in the container
        try:
            # Use a new client to be safe
            assert container.id is not None

            new_client = docker.from_env()
            new_container = new_client.containers.get(container.id)
            new_container.exec_run(cmd="kill -9 $(pidof python)")
        except Exception as e:
            logger.error(f"Failed to kill Python process in container: {e}")

        return -1, "Execution timed out after 150 seconds", ""
