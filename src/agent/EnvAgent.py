...
class EnvAgent(BaseAgent):
    ...
    def gen_multi_sp_egc(
        ...
        for i in range(count):
            ...
            # env_getter_code, succeed, new_tch = self.gen_single_sp_egc(
            env_getter_code, succeed, new_tch = self.gen_single_sp_egc_forced_guidance(
                count=i,
    ...
    def gen_multi_sp_egc_3(
        ...
        env_getter_code, succeed, new_tch = self.gen_single_sp_egc_forced_output(
            "```\n" + ip_addr_code.strip() + "\n```",
            ip_addr_code.strip(),
            count=0,
            genner=genner,
        ...
        if succeed:
            logger.info(
                f"EA - {0}-th SP EGC - Appending env_agent with new env getter code."
            )
            local_tch = local_tch + new_tch
            sp_egc.append(env_getter_code)
        ...
        if succeed:
            logger.info(
                f"EA - {1}-th SP EGC - Appending env_agent with new env getter code."
            )
            local_tch = local_tch + new_tch
            sp_egc.append(env_getter_code)
        ...
        if succeed:
            logger.info(
                f"EA - {2}-th SP EGC - Appending env_agent with new env getter code."
            )
            local_tch = local_tch + new_tch
            sp_egc.append(env_getter_code)
